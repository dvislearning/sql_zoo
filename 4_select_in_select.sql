...
SELECT continent, name FROM world x WHERE name = (SELECT name FROM world y WHERE y.continent = x.continent GROUP BY name LIMIT 1);
...
WHERE population > ALL (SELECT population*3 FROM world b WHERE a.continent = b.continent)
